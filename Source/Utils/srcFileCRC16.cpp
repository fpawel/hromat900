#include <string>
#include <vector>
#include <fstream>
#include <cassert>
#include <iostream>
#include <sstream>

//vcl
#include <system.hpp>

//---------------------------------------------------------------------------

#pragma hdrstop

//---------------------------------------------------------------------------

const unsigned char auchCRCHi[] =
    {
	
    };

    const unsigned char auchCRCLo[] =
    {
	
    };



//----------------------------------------------------------------
// Функция подсчёта контрольной суммы CRC блока данных
//----------------------------------------------------------------
unsigned short int CalculateCRC16(
    const unsigned char* pBuff, unsigned int buffLen,
    unsigned short int key)
{


	unsigned char
    // high byte of CRC initialized
	uchCRCHi = (key >> 8),
    // low byte of CRC initialized
	uchCRCLo = key;

    const unsigned char *i = pBuff, *end = pBuff + buffLen;

    //  pass through message buffer
	for( ; i!=end; ++i)
	{
        //  index into CRC lookup table
		const unsigned char
        uIndex = uchCRCHi ^ *i;
		//  calculate the CRC
		uchCRCHi = uchCRCLo ^ auchCRCHi[uIndex] ;
		uchCRCLo = auchCRCLo[uIndex] ;
	}
	return (uchCRCHi << 8) | uchCRCLo;
}
//------------------------------------------------------------------------------
std::size_t GetStreamSize(std::istream& strm)
{
	const std::size_t posG = strm.tellg();

	strm.seekg(0, std::ios::beg);
    const std::size_t pos1 = strm.tellg();

    strm.seekg(0, std::ios::end);
    const std::size_t pos2 = strm.tellg();

    strm.seekg(posG, std::ios::beg);

    return pos2-pos1;
}
//------------------------------------------------------------------------------
void LoadFileToBuff( const std::string& fn, std::vector<unsigned char>& buff  )
{
	// извлечь содержимое входного файла в буффер buff
	std::ifstream ifstrm( fn.c_str(), std::ios::binary );
    if( !ifstrm.is_open() ) return;
	const unsigned fileSize = GetStreamSize(ifstrm);
    buff.resize(fileSize);
    if(fileSize==0) return;

    char *pbBuffer = &buff.at(0); // адресс буфера
    ifstrm.read( pbBuffer, fileSize );
    assert( (unsigned)ifstrm.gcount()==fileSize );
    ifstrm.close();
}
//------------------------------------------------------------------------------

using std::cout;

#pragma argsused
int main(int argc, char* argv[])
{
	if(argc<2)
    {
    	cout << "No file name entererd!";
        return 0;
    }
    cout << "File name - \"" << argv[1] << "\"\n";

    std::vector<unsigned char> buff;
    LoadFileToBuff( argv[1], buff  );

    if( buff.empty() )
    {
    	cout << "File is empty or not exists!";
        return 0;
    }
    unsigned short offset = 0xFFFF;
    if(argc>2)
    {
    	std::stringstream sstrm(argv[2]);
    	sstrm  >> offset;
    }
    const unsigned short CRC16 = CalculateCRC16( &buff.at(0), buff.size(), offset);

    const AnsiString s =
    AnsiString().sprintf(
    "Offset = %d 0x%x\n"
    "CRC16  = %d 0x%x\n",
    offset, offset, CRC16, CRC16);
    cout << s;

    std::system("PAUSE");
	return 0;
}
//---------------------------------------------------------------------------
