...
//------------------------------------------------------------------------------
void __fastcall FTPHlpr::ConnectThreadProcedure()
{
    connectThreadLastError_ = 0;
    connectionWasBreaked_ = 0;
    hConnect_ = NULL;
    hConnect_ = ::InternetConnect(hInet_, serverName_.c_str(),
    	21, userName_.c_str(), userPassword_.c_str(), INTERNET_SERVICE_FTP,
        0, cntxt_);
    if( hConnect_==NULL )
        connectThreadLastError_ = ::GetLastError();
}
//------------------------------------------------------------------------------
        ...
        const AnsiString& userPassword)
{
    ...
    if( hConnect_==NULL )
    {
        if(connectThreadLastError_!=0)
            throw MyExcptionFactory::Create<MyWindowsException>(
                connectThreadLastError_, __FILE_LINE__,"Ошибка FTP::Connect");
        return;
    }
