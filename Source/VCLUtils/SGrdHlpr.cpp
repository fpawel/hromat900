...
AnsiString RowsSelectionInfoToStr(TStringGrid *grd)
{
    ...
    return my::BitsetToStrF( rsNfo ).c_str();
...
void SetRowsSelectionInfo(TStringGrid *grd, const AnsiString& strRsNfo)
{
	const int rc = grd->RowCount;
	if( rc==0 ) return;
	BitSet rsNfo = StrFToBitset( strRsNfo.c_str() );
    rsNfo.resize(rc-1,false);
    const unsigned sz = rsNfo.size();
    for( unsigned i=0; i<rc-1; ++i )
...
//------------------------------------------------------------------------------
void NormalizeGridHeight(TStringGrid *grd)
{
    grd->Height = grd->DefaultRowHeight*grd->RowCount + 10;
}
//------------------------------------------------------------------------------
